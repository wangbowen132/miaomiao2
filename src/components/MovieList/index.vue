      ...
      <ul ref="ul_node">
        ...
        <li v-for="movie in (newlist.length > 0 ? newlist : moviedata)" :key="movie.id">
          ...
          <div class="info_list">
            ...
            <p v-if="movie.comingTitle"> <span class="grade">{{movie.wish}}</span> 人想看</p>
            <p>主演: {{movie.star}}</p>
            <p v-if="!movie.comingTitle">{{movie.showInfo}}</p>
            <p v-if="movie.comingTitle">{{movie.rt}}上映</p>
          </div>
          <div :class="sellclass">
            {{sellname}}
          </div>
